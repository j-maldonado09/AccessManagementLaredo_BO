...
namespace AccessManagementLaredo
{
	...
	//                                  Specific Interface.
	// *********************************************************************************************
	public interface IAttachmentTypeRepository : Interfaces.IRepository<AttachmentType>
	{
	}
	...
	public class AttachmentTypeRepository : IAttachmentTypeRepository
	{
		...
		//                  Constructor.
		// ---------------------------------------------------------------------------------------------
		public AttachmentTypeRepository(Interfaces.IUnitOfWork unitOfWork)
		{
		...
		//                  Insert a new record.
		// ---------------------------------------------------------------------------------------------
		public int Create(AttachmentType entity)
		{
			ConvertCase(entity);

			_strQuery.Clear();
			_strQuery.Append("INSERT INTO ATCHMT_TYPE (");
			_strQuery.Append("ATCHMT_TYPE_NM, ");
			_strQuery.Append("ATCHMT_TYPE_DSCR");
			_strQuery.Append(") OUTPUT inserted.ATCHMT_TYPE_ID ");
		...
		{
			_strQuery.Clear();
			_strQuery.Append("DELETE FROM ATCHMT_TYPE WHERE ");
			_strQuery.Append("ATCHMT_TYPE_ID = @prm_id");
		...
		//                  Update record.
		// ---------------------------------------------------------------------------------------------
		public void Update(AttachmentType entity, int id)
		{
			ConvertCase(entity);

			_strQuery.Clear();
			_strQuery.Append("UPDATE ATCHMT_TYPE SET ");
			_strQuery.Append("ATCHMT_TYPE_NM = @prm_name, ");
			_strQuery.Append("ATCHMT_TYPE_DSCR = @prm_description ");
		...
		public string Read(int? id = -1)
		{
			...
			_strQuery.Append("ATCHMT_TYPE_ID AS Id, ");
			_strQuery.Append("ATCHMT_TYPE_NM AS Name, ");
			_strQuery.Append("ATCHMT_TYPE_DSCR AS Description ");
			_strQuery.Append("FROM ATCHMT_TYPE ");
		...
		// ---------------------------------------------------------------------------------------------
		private static void ConvertCase(AttachmentType entity)
		{
